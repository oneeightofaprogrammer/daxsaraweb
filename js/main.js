...
function initEventNest() {
  ...
  function update() {
    ...
    let activeLine = Math.min(n - 1, Math.floor(progress * n + 1e-6));
    if (progress >= 0.999) activeLine = n - 1;
    const seg = 1 / n;
    const local = (progress - activeLine * seg) / seg;
    const show = progress < 0.98 && local > 0.08 && local < 0.92;
    lines.forEach((line, i) => {
      line.classList.toggle("is-visible", show && i === activeLine);
    });
...
function initAcademyDev() {
  ...
  const rings = Array.from(section.querySelectorAll(".academy-dev__ring"));
  const clamp01 = v => Math.max(0, Math.min(1, v));
  ...
  function update() {
    ...
    rings.forEach((ring, i) => {
      const base = 0.82 + i * 0.06;
      const pulse = Math.sin(progress * Math.PI * 2 + i) * 0.03;
      const scale = base + progress * 0.22 + pulse;
      const rot = (progress * 12 - i * 4);
      ring.style.transform = `scale(${scale}) rotate(${rot}deg)`;
      ring.style.opacity = String(0.55 + progress * 0.35);
    });
    ...
    steps.forEach((step, i) => {
      const on = i === active;
      const exiting = !on && i === lastActive && lastActive !== -1 && lastActive !== active;
      step.classList.toggle("is-visible", on);
      step.classList.toggle("is-exit", exiting);
      if (!on && !exiting) {
        step.classList.remove("is-visible", "is-exit");
      }
    });
    ...
    if (yearBg && steps[active]) {
      ...
      if (m && yearBg.textContent !== m[1]) {
        yearBg.style.opacity = "0";
        setTimeout(() => {
          yearBg.textContent = m[1];
          yearBg.style.opacity = "1";
        }, 220);
      }
    }
